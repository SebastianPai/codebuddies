...
- `src/i18n/` — `LanguageContext` carga `dictionary[lang]`, que mergea ~15 namespaces (`admin`, `site`, `dashboard`, `gamification`, etc.) cada uno con `en.json`/`es.json`/`de.json`. **Todos los namespaces están siempre disponibles en cualquier componente** vía `t("namespace.key", params)` — no hace falta "importar" un namespace por página. Placeholders con `{nombre}`, no `{{nombre}}`. Nota de calidad: `admin/es.json` tiene texto en **inglés** en varias claves preexistentes (deuda de traducción, no introducida por este documento) — las claves nuevas que se agreguen ahí deben traducirse de verdad, no copiar el inglés.
